// PUT /api/admin-hero-stats — SUPER_ADMIN only, top-level Vercel function
// GET /api/admin-hero-stats — returns current values
...
async function handleGet(req, res) {
  ...
  try {
    ...
    console.error('[admin-hero-stats get]', e.message);
  }
...
async function handlePut(req, res) {
  ...
  for (const k of KEYS) {
    const v = body[k];
    if (typeof v === 'string') toSave[k] = v.slice(0, 32);
    else if (v != null) toSave[k] = String(v).slice(0, 32);
  }
  try {
    if (prisma.systemSetting) {
    ...
    console.error('[admin-hero-stats put]', e.message);
    return error(res, 500, 'Failed to save');
  }
  return json(res, 200, { success: true, saved: toSave });
}
