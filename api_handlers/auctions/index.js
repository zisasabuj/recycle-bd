...
// Lazy expiry: skip the status update (Prisma client enum doesn't match DB enum).
// The list query already filters with `endsAt: { gt: new Date() }` so expired
// items are never returned. A future migration can clean them up.
